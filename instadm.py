...
class InstaDM(object):
	...
	def login(self, username, password):
		...
		self.driver.find_elements_by_xpath("//*[text()[contains(., 'Log In')]]")[0].click()
		...
		self.driver.find_elements_by_xpath("//*[text()[contains(., 'Log In')]]")[0].click()
		sleep(5)
		self.driver.find_elements_by_xpath("//*[text()[contains(., 'Not now')]]")[0].click()
   
	def sendMessage(self, user, message, qtd):
		...
		self.driver.find_element_by_name('queryBox').send_keys(user)
		self.__randomSleep__()

		# Select user
		#elements = self.driver.find_elements_by_css_selector(self.selectors['select_user'])
		self.driver.find_elements_by_xpath("//*[text()[contains(., '{}')]]".format(user))[0].click()
		self.__randomSleep__()
  
		self.driver.find_elements_by_xpath("//*[text()[contains(., 'Next')]]")[0].click()
		
		self.__randomSleep__()
		self.trava(user, message, qtd)
 		# self.__randomSleep__()
	...
	def trava(self, user, message, qtd):
		message = wrap(message, 128)
  
		for i in range(len(message)):
			self.driver.find_elements_by_xpath("*//textarea")[0].send_keys(message[i])
			sleep(1)
			buttons = self.driver.find_elements_by_css_selector(self.selectors['send'])
			buttons[len(buttons)-1].click()

			if self.conn != None:
				self.cursor.execute('INSERT INTO message (username, message) VALUES(?, ?)', (user, message[i]))
				self.conn.commit()
			sleep(1)
	...
	def __randomSleep__(self, min = 2, max = 6):
		t = randint(min, max)
